...
#     endpoint = reverse_lazy("manage-zaak-document")
...
class ZaakDocumentResponseTests(ClearCachesMixin, APITransactionTestCase):
    ...
    @requests_mock.Mocker()
    def test_patch_document_no_url(self, m):
        user = SuperUserFactory.create()
        self.client.force_authenticate(user)
        self._setupMocks(m)

        response = self.client.patch(self.endpoint, self.post_data, format="multipart")

        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    @requests_mock.Mocker()
    def test_patch_document_wrong_zaak(self, m):
        user = SuperUserFactory.create()
        self.client.force_authenticate(user)
        self._setupMocks(m)
        ...
        post_data = {**self.post_data, "document": "http://some-document.nl"}
        with patch(
    ...
    def test_patch_document(self, m):
        ...
        post_data = {
            **self.post_data,
            "document": f"{DOCUMENTS_ROOT}enkelvoudiginformatieobjecten/0c47fe5e-4fe1-4781-8583-168e0730c9b6",
        }
        with patch(
            "zac.core.api.serializers.get_documenten",
            return_value=[[factory(Document, document)], []],
